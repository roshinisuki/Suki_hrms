...
export async function GET(
  ...
  const record = await prisma.level.findFirst({ where: { id: parseInt(id), deletedAt: null } });
...
export async function PUT(
  ...
  const existing = await prisma.level.findFirst({ where: { code: parsed.data.code, NOT: { id: parseInt(id), deletedAt: null } } });
  if (existing && existing.deletedAt === null) return NextResponse.json({ error: 'Code already exists' }, { status: 409 });

  const record = await prisma.level.update({ where: { id: parseInt(id) }, data: parsed.data });
  return NextResponse.json(record);
}
