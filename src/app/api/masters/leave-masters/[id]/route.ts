...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const record = await prisma.leaveMaster.findFirst({ where: { id: parseInt(id), deletedAt: null } });
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const existing = await prisma.leaveMaster.findFirst({ where: { code: parsed.data.code, NOT: { id: parseInt(id) } } });
...
export async function DELETE(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  await prisma.leaveMaster.update({ where: { id: parseInt(id) }, data: { deletedAt: new Date(), isActive: false } });
