...
export async function GET(request: NextRequest) {
  ...
  const [data, total] = await Promise.all([
    ...
    prisma.professionalTaxSlab.count({ where }),
  ]);
...
export async function POST(request: NextRequest) {
  ...
  const parsed = professionalTaxSlabSchema.safeParse(body);
  ...
  const record = await prisma.professionalTaxSlab.create({ data: parsed.data });
