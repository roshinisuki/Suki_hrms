...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const record = await prisma.oTPlan.findFirst({ where: { id: parseInt(id), deletedAt: null } });
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const existing = await prisma.oTPlan.findFirst({ where: { code: parsed.data.code, NOT: { id: parseInt(id) } } });
  if (existing && existing.deletedAt === null) return NextResponse.json({ error: 'Code already exists' }, { status: 409 });

  const record = await prisma.oTPlan.update({ where: { id: parseInt(id) }, data: parsed.data });
  return NextResponse.json(record);
}
